...
### 新機能
- トップページのプロジェクト一覧をプロジェクト名の昇順に表示するよう変更 (hugo-theme-issues)
- 左右サイドバーの開閉・リサイズ・Cookie保存 (hugo-theme-issues)
  - トグルボタン（◀/▶）でサイドバーを開閉
  - ドラッグハンドルでサイドバー幅を150px〜500pxの範囲で変更
  - 状態（開閉・幅）をCookieに保存し、ページ遷移後も復元
  - モバイル(768px以下)ではトグル・リサイズを非表示
...
- 日付処理のリファクタリング
  ...
  - `formatCommentDate` を削除し `formatTimeString` に統合（同一ロジックの重複解消）
  - `formatRFC3339` 関数を追加（開発情報セクションのインラインパース処理を共通化）
  - `formatTime` に `.Local()` を追加してシステムタイムゾーンに変換
  - 日付フォーマットを `2006-01-02 15:04:05` に統一

### 新機能
- 日付フォーマットの統一とタイムゾーン対応
  - すべての日付表示を `2006-01-02 15:04:05` 形式に統一（期限・Front Matterを除く）
  - システムタイムゾーンに変換して表示（`.Local()`）
  - コメント日付のフォールバック処理を削除
  - 変更履歴の日付パースをJIRA形式に統一
- PRの更新日時を取得・表示
...
- JIRAのコードブロック（`{code}`/`{noformat}`）内にMarkdownのコードブロック記法（` ``` `）が含まれる場合、変換後にコードブロックが意図しない位置で閉じられる問題を修正
  - CommonMarkの仕様に従い、外側フェンスのバッククオート数を内側の最長連続バッククオートより1つ多くする `fenceForContent` 関数を追加
  - `{code:lang}`, `{code}`, `{noformat}` の3パターン全てに対応
...
- JIRAの見出し（h2.～h6.）がMarkdown変換後にプレースホルダーとして残る問題を修正
  ...
  - プレースホルダーのインデックス不一致を修正
- 古いJIRAのリスト形式（先頭にスペースが入る` * `や` # `）に対応
- カスタムフィールド内の開発情報（repository情報）が基本情報セクションに表示される問題を修正
  - `repository=`を含む文字列も開発フィールドとして認識するように改善
  - より汎用的な`dataType=`と`count=`のパターンも追加

### 改善
- 開発情報セクションの表示順序をJIRA仕様に合わせた
  - ブランチ → プルリクエストの順序で出力（従来はプルリクエスト → ブランチ）
  - JIRAのUI表示順序と一致するように修正
- ブランチの最終コミット情報をMarkdownに出力
  - GraphQL APIで取得した`lastCommit`情報（コミットハッシュ、タイムスタンプ）を表示
  - ブランチセクション内にコミット情報を追加表示
  - 表示形式: `- 最終コミット: [`abc1234`](url) (2025-01-15 14:30:05)`
- 開発情報取得（Dev-Status API）の詳細なデバッグログを追加
  - `LOG_LEVEL=DEBUG` 環境変数を設定することで `debug.log` に詳細なログを出力
  - リクエスト詳細（URL、パラメータ）、レスポンス詳細（ステータスコード、ボディ、実行時間）、エラー詳細を記録
  - 開発情報が取得できないケースの原因調査に活用
- テーブル内のJIRAリスト要素をHTMLリストに変換
  - `*` で始まる箇条書きは `<ul><li>` に変換
  - `#` で始まる番号付きリストは `<ol><li>` に変換
  - ネストされたリストにも対応
  - Markdownレンダラーでリストとして正しく表示される

### 変更
- RankフィールドのカスタムフィールドIDを設定可能に
...
- コメントの出力形式を改善
  - 昇順（古い順）で出力
  - タイトルを「投稿者名 投稿日」形式に変更（`###`見出しから`---`区切りに変更）
  - 投稿日を `yyyy-mm-dd hh:mm` 形式に変更
  - 返信コメントに ↩️ マークを付与
