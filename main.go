...
func fetchIssue(ctx context.Context, cmd *cli.Command) error {
	...
	devStatus, devStatusRawJSON := fetchDevStatus(jiraClient, config, issueKey, issue.ID)
	...
	childIssues, err := fetchAndSortChildIssues(jiraClient, config, issue.Key)
	if err != nil {
		fmt.Printf("警告: %v\n", err)
		childIssues = nil
	}
...
func searchIssues(ctx context.Context, cmd *cli.Command) error {
	...
	for i, issueKey := range issueKeys {
		...
		if cachedChildren, exists := childIssuesCache[issue.Key]; exists {
			...
			fetched, err := fetchAndSortChildIssues(jiraClient, config, issue.Key)
			if err != nil {
				fmt.Printf("  警告: %v\n", err)
			} else {
				childIssues = fetched
